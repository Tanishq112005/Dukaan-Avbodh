...
export function ProductInfo({ product, addToCart }) {
  ...
  return (
    <div className="md:w-1/2 py-4">
      {product.brand && <p className="text-[#C45C26] font-bold uppercase tracking-[0.2em] text-xs mb-2">{product.brand}</p>}
      <h1 className="font-display text-4xl font-extrabold mb-2 uppercase">{product.name}</h1>
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap gap-3">
          {productSizes.map(size => (
            <button key={size} onClick={() => setSelSize(size)} className={`px-6 py-3 rounded-full transition-colors ${selSize === size ? 'bg-stone-900 text-white' : 'bg-[#EFE8DE] text-stone-500 hover:bg-[#e4d9cc]'}`}>{size}</button>
          ))}
        </div>
      </div>
      <hr className="my-6" />
      <div className="flex gap-4">
        <div className="flex items-center justify-between bg-[#EFE8DE] rounded-full px-6 py-4 w-40">
          <Minus className="cursor-pointer" onClick={() => setQty(Math.max(1, qty - 1))} />
          <span className="font-bold">{qty}</span>
          <Plus className="cursor-pointer" onClick={() => setQty(qty + 1)} />
        </div>
        <Button className="flex-1 rounded-full py-6 text-lg bg-stone-900 hover:bg-stone-800" onClick={() => addToCart(qty, selSize, selColor)}>Add to Cart</Button>
      </div>
    </div>
  );
}
