...
export function MerchantAnalytics() {
  ...
  useEffect(() => {
    ...
    const fetchAnalytics = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_API_URL || "http://localhost:8000"}/merchant/analytics`, {
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          ...
          <p className="text-3xl font-black mt-2 text-green-600">₹{metrics.total_revenue}</p>
        </div>
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <p className="text-sm font-semibold text-gray-500 uppercase">Total Profit</p>
          <p className="text-3xl font-black mt-2">₹{metrics.total_profit}</p>
          <p className="text-xs text-gray-400 mt-1">Margin: {metrics.profit_margin_percent}%</p>
        </div>
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        ...
        <div className="bg-white p-6 rounded-xl border border-red-200 shadow-sm bg-red-50">
          <p className="text-sm font-semibold text-red-600 uppercase">Discounts by AI</p>
          <p className="text-3xl font-black mt-2 text-red-700">₹{metrics.total_ai_discount_amount}</p>
        </div>
      </div>
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={revenue_trend}>
                ...
                <Line type="monotone" dataKey="discount_given" stroke="#dc2626" strokeWidth={2} name="AI Discount (₹)" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
