      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Transparent logs of all AI negotiations, profit margins, and strict
            bounds enforcement.
          </p>
        </div>
      </div>
      {loading ? (
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {logs.map((log) => (
            <div
              key={log._id}
              className="bg-gray-100 p-5 rounded-lg text-sm border-l-4 border-black"
            >
              <div className="flex justify-between items-start mb-3">
                <span className="font-bold text-black uppercase tracking-wider bg-white px-2 py-1 rounded text-xs">
                  {log.action}
                </span>
