...
async def get_all_orders(
    ...
    async with db_connection.get_session() as session:
        ...
        statement = select(Order).options(joinedload(Order.product), joinedload(Order.user)).order_by(Order.created_at.desc()).limit(50)
        results = await session.execute(statement)
        orders = results.scalars().all()
        
        # Clean up data to return, grouping by razorpay_order_id
        grouped_orders = {}
        for order in orders:
            group_id = order.razorpay_order_id or str(order.id)
            ...
            if order.product:
                grouped_orders[group_id]["products"].append({
